...
def des_decrypt(bits, key):
    """ Descifrar un bloque de texto usando DES. """
    # Convertir el texto y la clave en bits
    key = text_to_bits(key)
    ...
    for i in range(0, len(bits), 64):
        block = bits[i:i + 64]
        decrypted_bits += des_decrypt_block(block, subkeys)

    return bits_to_text(decrypted_bits)

def des_decrypt_block(block, keys):
    """ Proceso completo de descifrado DES en 16 rondas para un bloque de 64 bits. """
    permuted_block = permutate(block, IP)
    l_half, r_half = permuted_block[:32], permuted_block[32:]
